// CryptoKiddie — injectable `cadesplugin` emulation for the NEW unified ФНС
// cabinet ЕЛК (https://elk.nalog.gov.ru/login), loaded NATIVELY in the browser.
// The ЕЛК React bundle does `if(!window.cadesplugin){ load real КриптоПро loader }`,
// so injecting this shim as `window.cadesplugin` before the cert-login runs makes
// the app route signing to the local token bridge instead of the extension.
// The page is a regular-TLS origin, so signing is delegated CROSS-ORIGIN to the
// bridge's /__bridge/sign (absolute URL, no-preflight: no Content-Type header, so
// it stays a CORS-simple request; the bridge parses the JSON body regardless).
// Inject (before choosing "по сертификату"):
//   osascript -e 'tell application "Safari" to do JavaScript (read POSIX file ".../elk-inject.js") in front document'
(function(){
if(window.__ckCadesShim)return;window.__ckCadesShim=true;
var B='http://127.0.0.1:18889/__bridge';
// Diagnostic: record failing fetch/XHR (status>=400 or network error).
...
return fetch(B+'/sign',{method:'POST',cache:'no-store',
...
// Force our shim into place even if the ЕЛК loader already set a (failed) one.
try{delete window.cadesplugin;}catch(e){}
...
console.log('[CryptoKiddie] cadesplugin emulation injected (ЕЛК elk.nalog.gov.ru, absolute bridge oracle '+B+')');
})();
